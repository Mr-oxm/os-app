    ...
    return (
        <div className="w-full h-full p-4 flex flex-col rounded-lg shadow-md bg-gray-100">
            ...
            <div className="grid grid-cols-5 gap-2 flex-grow">
                ...
                {['0', '.', '=', '+', 'MC'].map((btn) => (
                    <Button key={btn} onClick={() => btn === '=' ? handleEquals() : ['MC', '+'].includes(btn) ? (btn === 'MC' ? handleMemory(btn) : handleOperationClick(btn)) : handleNumberClick(btn)} className={`h-full ${['=', 'MC', '+'].includes(btn) ? 'bg-purple-500 hover:bg-purple-600' : 'bg-orange-500 hover:bg-orange-600'}`}>{btn}</Button>
                ))}
                ...
                <Button onClick={handleRemoveLast} className="col-span-5 h-full bg-gray-500 hover:bg-gray-600">⌫ Remove Last</Button>
            </div>
        </div>
    )
}
