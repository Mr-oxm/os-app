...
const MacWindow = ({
    ...
    const windowComponents=[
        <MacButtons handleMaximize={handleMaximize}/>,
        <WindowsButtons handleMaximize={handleMaximize}/>,
    ]
...
const MacButtons= ({ handleMaximize })=>{return(
    <div className={`flex ${windowDir==0?"flex-row": "flex-row-reverse"} gap-2 space-x-2 text-window-foreground w-1/4`}>
        <Link href={"/"} onClick={} className="w-3 h-3 bg-window-exit rounded-full group overflow-hidden">
...
const WindowsButtons= ({ handleMaximize })=>{return(
