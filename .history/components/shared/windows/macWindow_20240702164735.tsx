...
const MacWindow = ({
    ...
    return (
        <div
            className={`card !p-0 bgblur overflow-hidden bg-background absolute ${isMaximized ? 'w-full h-full' : 'w-1/2 h-1/2'} p-4 ${isDragging?"":"transition-all"} ease-in-out`} 
            style={{ top: position.top, left: position.left }}
        >
            <div className={`flex justify-between p-2 bg-blue-400 items-center mb-2 ${isMaximized ? 'cursor-default' : isDragging ? 'cursor-grabbing' : 'cursor-grab'}`} onMouseDown={handleMouseDown}>
