...
const MacWindow = ({
    ...
    const handleMaximize = useCallback(() => {
        if (isMaximized) {
            ...
            setSize({ width: '50%', height: '50%' }); // Or any default size
        } else {
            setIsMaximized(true);
    ...
    const handleResize = useCallback((e: React.MouseEvent, direction: string) => {
        ...
        const startWidth = parseInt(size.width as string);
        const startHeight = parseInt(size.height as string);
    
        const doDrag = (e: MouseEvent) => {
            if (direction.includes('e')) {
                setSize(prevSize => ({ ...prevSize, width: `${startWidth + e.clientX - startX}px` }));
            }
            if (direction.includes('s')) {
                setSize(prevSize => ({ ...prevSize, height: `${startHeight + e.clientY - startY}px` }));
            }
        };
    ...
    return (
        ...
        >
            <div 
                className={`flex ${windowDir==0?"flex-row": "flex-row-reverse"} justify-between p-2 h-8 bg-background items-center ${isMaximized ? 'cursor-default' : isDragging ? 'cursor-grabbing' : 'cursor-grab'}`} 
            ...
            {!isMaximized && (
                <>
                    <div className="absolute top-0 right-0 w-2 h-full cursor-ew-resize" onMouseDown={(e) => handleResize(e, 'e')} />
                    <div className="absolute bottom-0 left-0 w-full h-2 cursor-ns-resize" onMouseDown={(e) => handleResize(e, 's')} />
