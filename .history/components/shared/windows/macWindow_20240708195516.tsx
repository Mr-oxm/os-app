...
const MacButtons = ({ handleExit, handleMaximize, windowDir}:ButtonProps) => (
    <div className={`flex ${windowDir==0?"flex-row": "flex-row-reverse"} gap-2 space-x-2 text-foreground w-1/4`}>
        <Link href={"/"} onClick={handleExit} className="w-3 h-3 bg-window-exit rounded-full group overflow-hidden text-foreground">
...
const WindowsButtons = ({ handleExit, handleMaximize, windowDir}:ButtonProps) => (
