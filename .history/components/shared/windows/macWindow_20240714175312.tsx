...
const MacWindow = ({
    ...
    return (
        ...
        >
            <div className={`prevent-select flex ${windowDir==0?"flex-row": "flex-row-reverse"} justify-between p-2 h-8 bg-background items-center ${isMaximized ? 'cursor-default' : isDragging ? 'cursor-grabbing' : 'cursor-grab'}`} onMouseDown={handleMouseDown} onClick={()=>setActive(programId)}>
                <ButtonComponent 
                ...
                />
                <div className="flex-grow text-center text-foreground text-xs font-semibold capitalize prevent-select">
                    {programId}
                </div>
