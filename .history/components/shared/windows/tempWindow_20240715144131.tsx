    ...
    <div className={`flex ${windowDir==0?"flex-row": "flex-row-reverse"} gap-2 space-x-2 text-window-foreground w-1/4 items-center`}>
        ...
        <TooltipProvider>
            <Tooltip>
                ...
                <WindowSplit handleSplit={handleSplit}/>
            </Tooltip>
        </TooltipProvider> 
    </div>
);
...
const WindowSplit=({handleSplit}:{handleSplit:(direction:string)=>void})=>{
    return(
        <TooltipContent className={`card flex flex-col gap-2 ${windowD}translate-x-7 !text-xs `}>
            <h2 className='col-span-2 text-xs'>Split in Two</h2>
            <div className='grid grid-cols-2 gap-2'>
                <Button onClick={()=>{handleSplit('left')}} className='rounded-r-none h-20 !text-xs !text-primary-foreground'>Left</Button>
                <Button onClick={()=>{handleSplit('right')}} className='rounded-l-none h-20 !text-xs !text-primary-foreground'>Right</Button>
            </div>
            <h2 className='col-span-2 text-xs'>Split in Four</h2>
            <div className='grid grid-cols-2 gap-2 h-fit'>
                <div className='flex flex-col gap-2 h-full '>
                    <Button onClick={()=>{handleSplit('NW')}} className='rounded-r-none rounded-b-none !text-xs !text-primary-foreground'>NW</Button>
                    <Button onClick={()=>{handleSplit('SW')}} className='rounded-r-none rounded-t-none !text-xs !text-primary-foreground'>SW</Button>
                </div>
                <div className='flex flex-col gap-2 h-full'>
                    <Button onClick={()=>{handleSplit('NE')}} className='rounded-l-none rounded-b-none !text-xs !text-primary-foreground'>NE</Button>
                    <Button onClick={()=>{handleSplit('SE')}} className='rounded-l-none rounded-t-none !text-xs !text-primary-foreground'>SE</Button>
                </div>
            </div>
        </TooltipContent>
    )
