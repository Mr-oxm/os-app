...
const MacWindow = ({
    ...
    const MacButtons = ({ handleMaximize }) => {
        return (
    ...
    const WindowsButtons = ({ handleMaximize }) => {
        return (
            <div className={`flex ${windowDir === 0 ? "flex-row" : "flex-row-reverse"} gap-2 text-window-foreground w-1/4`}>
                <Link href={"/"} onClick={handleExit}>
                    <Button className="p-1 w-8 h-8 bg-transparent hover:!bg-window-exit rounded-md overflow-hidden !m-0">
                        <X className='w-4 h-4'/> 
                    </Button>
                </Link> 
    ...
    const windowComponents = [
        <MacButtons key="mac" handleMaximize={handleMaximize} />,
        <WindowsButtons key="windows" handleMaximize={handleMaximize} />,
    ]
...
const MacButtons = ({ handleExit, handleMaximize, handleMinimize }) => (
    <div className="flex flex-row gap-2 space-x-2 text-window-foreground w-1/4">
...
const WindowsButtons = ({ handleExit, handleMaximize, handleMinimize }) => (
    <div className="flex flex-row gap-2 text-window-foreground w-1/4">
        <Button onClick={handleMinimize} className="p-1 w-8 h-8 bg-transparent hover:!bg-foreground/15 rounded-md overflow-hidden !m-0">
            <Minus className='w-4 h-4'/>
        </Button>
        <Button onClick={handleMaximize} className="p-1 w-8 h-8 bg-transparent hover:!bg-foreground/15 rounded-md overflow-hidden !m-0">
            <Maximize2 className='w-4 h-4 '/> 
        </Button>
        <Button onClick={handleExit} className="p-1 w-8 h-8 bg-transparent hover:!bg-window-exit rounded-md overflow-hidden !m-0">
            <X className='w-4 h-4'/> 
        </Button>
    </div>
);
