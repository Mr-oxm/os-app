...
const MacWindow = () => {
    ...
    return (
        <div
            className={`card bgblur bgOpacity absolute ${isMaximized ? 'w-full h-full' : 'w-1/2 h-1/2'} p-4 top-[${position.top}] left-[${position.left}]`}
            style={{
                position: 'absolute',
                top: position.top,
                left: position.left,
        ...
        >
            <div className={`flex justify-between items-center mb-2 ${isMaximized ? 'cursor-default' : isDragging ? 'cursor-grabbinggrabbing' : 'grab'}`} onMouseDown={handleMouseDown}>
