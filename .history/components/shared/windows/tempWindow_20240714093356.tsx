...
const MacWindow = ({
    ...
    return (
        <div
            className={`card ${windowType==1?"!rounded-none": ""} ${minimized?"hidden":"flex"} flex-col !p-0 bgblur overflow-hidden bgOpacity absolute ${isMaximized ? 'w-full h-full' : 'w-1/2 h-1/2'} ${(active==)&&"z-20"} p-4 ${isDragging?"":"transition-all"} ease-in-out`} 
