...
const MacWindow = ({
    ...
    const handleMouseMove = useCallback((e: MouseEvent) => {
        if (isDragging && !isMaximized) {
            ...
            switch (resizeDirection) {
                case 'n':
                    newSize.height = `${rect.bottom - e.clientY}px`;
                    newPosition.top = e.clientY;
                    break;
                case 's':
                    newSize.height = `${e.clientY - position.top}px`;
                    break;
                case 'w':
                    newSize.width = `${rect.right - e.clientX}px`;
                    newPosition.left = e.clientX;
                    break;
                case 'e':
                    newSize.width = `${e.clientX - position.left}px`;
                    break;
                case 'nw':
                    newSize.width = `${rect.right - e.clientX}px`;
                    newSize.height = `${rect.bottom - e.clientY}px`;
    ...
    }, [isDragging, isResizing, offset, isMaximized, position, size, resizeDirection]);
    ...
    useEffect(() => {
        const element = document.getElementById(`window-${programId}`);
        if (element) {
    ...
    }, [isDragging, isResizing, handleMouseMove, programId]);
    ...
    return (
        <div
            id={`window-${programId}`}
            className={`card ${windowType == 1 ? "!rounded-none" : ""} ${minimized ? "hidden" : "flex"} flex-col !p-0 bgblur overflow-hidden bgOpacity absolute ${isMaximized ? 'w-full h-full' : ''} ${(active === programId) && "z-20"} p-4 ${isDragging||isResizing ? "" : "transition-all"} ease-in-out`}
            style={{ top: position.top, left: position.left, width: size.width, height: size.height }}
        >
            <div className={`flex ${windowDir == 0 ? "flex-row" : "flex-row-reverse"} justify-between p-2 h-8 bg-background items-center ${isMaximized ? 'cursor-default' : isDragging ? 'cursor-grabbing' : 'cursor-grab'}`} onMouseDown={handleMouseDown} onClick={() => setActive(programId)}>
                <ButtonComponent
            ...
            {!isMaximized && (
                <>
                    <div className="absolute top-0 left-0 right-0 h-1 cursor-n-resize" onMouseDown={handleResizeStart('n')}></div>
                    ...
                    <div className="absolute top-0 left-0 w-3 h-3 cursor-nw-resize" onMouseDown={handleResizeStart('nw')}></div>
                    <div className="absolute top-0 right-0 w-3 h-3 cursor-ne-resize" onMouseDown={handleResizeStart('ne')}></div>
