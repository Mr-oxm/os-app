...
const WindowsButtons = ({ handleExit, handleMaximize, handleMinimize, windowDir, windowType, handleSplit }:ButtonProps) => (
    <div className={`flex ${windowDir==0?"flex-row": "flex-row-reverse"} gap-2 !text-foreground w-1/4 items-center`}>
        <Button onClick={handleExit} className={`p-1 w-6 h-6 bg-transparent hover:!bg-window-exit rounded-md overflow-hidden !text-foreground !m-0 ${windowType==2&&"bg-window-exit rounded-full"} ${windowType==3&&"bg-window-exit rounded-full"}`}>
            {windowType===3? 
                <ImCross />
                :
                <X className='w-4 h-4'/> 
