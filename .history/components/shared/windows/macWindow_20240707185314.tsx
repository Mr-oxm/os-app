    ...
    const WindowsButtons= ()=>{return(
        <div className={`flex ${windowType==0?"flex-row": "flex-row-reverse"} gap-2 space-x-2 text-window-foreground w-1/4`}>
            <Link href={"/"} onClick={handleExit}>
                <Button className="p-1 w-6 h-8 bg-transparent hover:!bg-foreground/15 rounded-md overflow-hidden !m-0">
                    <X className='w-full h-full'/> 
                </Button>
            </Link> 
            <Button onClick={handleMaximize} className="p-1 w-6 h-8 bg-transparent hover:!bg-foreground/15 rounded-md overflow-hidden !m-0">
