...
const MacWindow = ({
    ...
    const [lastPosition, setLastPosition] = useState<{ top: number, left: number | string }>({ top: 0, left: 0 });
    const [isDragging, setIsDragging] = useState(false);
    const [isResizing, setIsResizing] = useState(false);
    const [offset, setOffset] = useState({ x: 0, y: 0 });
    const [size, setSize] = useState<{ width: number, height: number | string }>({ width: '100%', height: '100%' });
