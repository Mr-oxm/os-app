...
const FullDesktop = ({ children }: { children: React.ReactNode }) => { 
    const { mainbodyType, taskbarType, topbarType, wallpaper, firstboot, taskbarPos,taskbarDir} = useAppStore()
    ...
    const taskbarPosition=[
        ...
        <HoverCard>
            ...
            `text-background  absolute rounded-full border-white  blur-2xl transition-all 
            ${
                taskbarDir==0? 
                "bottom-0 w-3/5 h-8 hover:border-b-8"
                :
                ( taskbarDir==1? 
                "bottom-0 right-0 h-40  w-8 hover:border-r-8"
                :"left-0 h-4/5 w-8 hover:border-l-8")
            }`
            }>
            </HoverCardTrigger>
            <HoverCardContent className='bg-transparent w-fit border-0 shadow-none h-fit p-0 rotate-90'>
                {taskBarComponents[taskbarType]} 
            </HoverCardContent>
        </HoverCard>
    ]
