...
// Assuming you have a similar constant for folder icons
const FoldersIcons = [
  { 
    documents: {
      light: '/path/to/documents-light.png',
      dark: '/path/to/documents-dark.png'
    }
  },
  { 
    pictures: {
      light: '/path/to/pictures-light.png',
      dark: '/path/to/pictures-dark.png'
    }
  },
  { 
    music: {
      light: '/path/to/music-light.png',
      dark: '/path/to/music-dark.png'
    }
  },
];
...
const FinderApp: React.FC = () => {
  ...
  const [iconsType, setIconsType] = useState<'light' | 'dark'>('light');
  ...
  const getImgSrc = (id: string): string => {
    ...
    if (folder) {
      return folder[id as keyof typeof folder][iconsType];
    }
    return '';
  };
      ...
      <div className="p-2 flex items-center justify-between text-sm text-muted-foreground">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Slider
            ...
            className="w-24"
          />
          <Button variant="ghost" size="sm" onClick={() => setIconsType(iconsType === 'light' ? 'dark' : 'light')}>
            Toggle Theme
          </Button>
        </div>
      </div>
