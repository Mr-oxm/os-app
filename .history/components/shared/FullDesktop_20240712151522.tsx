...
const FullDesktop = ({ children }: { children: React.ReactNode }) => { 
    ...
    const taskbarPosition=[
        ...
        <HoverCard>
            <HoverCardTrigger className={`text-background  absolute rounded-full border-white hover:border-b-8 blur-2xl transition-all ${taskbarDir==0? "bottom-0 w-3/5":( taskbarDir==1? "right-0":"flex-row-reverse")}`}></HoverCardTrigger>
