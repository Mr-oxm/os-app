...
const dockDesktopIcon = ({imgSrc, name}:iIcons) => {
    ...
    return (
        <div onClick={handleClick} className={`item bg-transparent flex flex-col gap-4 items-center transition-all rounded-lg justify-center h-full w-full relative group ${isBouncing ? 'animate-bouncing' : ''} `}>
            <div className={` opacity-0 transition-all duration-200 ease-in-out absolute flex-col items-center group-hover:opacity-100 flex ${taskbarDir===0? "bottom-10":(taskbarDir==1?"right-10":"left-10")}`}>
                <p className=" text-xs w-24 text-center scale-75 bg-background py-1 px-4 rounded-lg z-50 ">
                    {name}
                </p>
