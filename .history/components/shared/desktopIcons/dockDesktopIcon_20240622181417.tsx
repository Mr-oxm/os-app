...
const dockDesktopIcon = ({imgSrc, name}:iIcons) => {
    ...
    return (
        <div onClick={handleClick} className={`item bg-transparent flex flex-col gap-4 items-center transition-all rounded-lg justify-center h-full w-full relative group ${isBouncing ? 'animate-bouncing' : ''} `}>
            <div className=" hidden absolute bottom-12 flex flex-col items-center justify-start group-hover:block">
                <p className=" text-xs scale-75 bg-white py-1 px-4 rounded-lg ">{name}</p>
                <div className="bottom-11 w-0 h-0 border-l-8 border-l-transparent border-t-8 border-t-white border-r-8 border-r-transparent"></div>
            </div>
