...
const FullDesktop = ({ children }: { children: React.ReactNode }) => { 
    ...
    const taskbarPosition=[
        ...
        <HoverCard>
            <HoverCardTrigger className={`text-background w-3/5 absolute bottom-0 h-8 rounded-full border-white hover:border-b-8 blur-2xl transition-all ${taskbarDir==0? "bottom-0":( taskbarDir==1? "flex-row":"flex-row-reverse")}`}></HoverCardTrigger>
