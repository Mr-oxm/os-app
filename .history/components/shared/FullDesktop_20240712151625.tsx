...
const FullDesktop = ({ children }: { children: React.ReactNode }) => { 
    ...
    const taskbarPosition=[
        ...
        <HoverCard>
            <HoverCardTrigger className={`text-background  absolute rounded-full border-white hover:border-b-8 blur-2xl transition-all ${taskbarDir==0? "bottom-0 w-3/5 h-8":( taskbarDir==1? "right-0 h-3/5 w-8 bg-blue-500":"flex-row-reverse")}`}></HoverCardTrigger>
