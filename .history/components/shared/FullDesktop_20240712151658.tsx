...
const FullDesktop = ({ children }: { children: React.ReactNode }) => { 
    ...
    const taskbarPosition=[
        ...
        <HoverCard>
            <HoverCardTrigger className={`text-background  absolute rounded-full border-white  blur-2xl transition-all ${taskbarDir==0? "bottom-0 w-3/5 h-8 hover:border-b-8":( taskbarDir==1? "right-0 h-3/5 w-8 hover:border-b-8":"flex-row-reverse")}`}></HoverCardTrigger>
