...
import { FaFile, FaHeart, FaTag, FaUpload, FaSort, FaCloudUploadAlt } from 'react-icons/fa';
...
const FileContextMenu: React.FC<{ children: React.ReactNode; item: FileItem; onDelete: () => void; onRename: () => void }> = 
    ...
    <ContextMenu>
        ...
        <ContextMenuContent className='card bgOpacity bgblur'>
            <ContextMenuItem className='text-cs' onClick={() => console.log(`Open ${item.name}`)}>Open</ContextMenuItem>
            <ContextMenuItem className='text-cs' onClick={onDelete}>Delete</ContextMenuItem>
            <ContextMenuItem className='text-cs' onClick={onRename}>Rename</ContextMenuItem>
        </ContextMenuContent>
    </ContextMenu>
);
...
const FinderApp: React.FC = () => {
    ...
    // Helper functions
    const getImgSrc = (): string => FoldersIcons[0].files[iconsType];
