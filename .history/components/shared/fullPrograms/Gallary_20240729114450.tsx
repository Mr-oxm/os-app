...
const Gallery = () => {
    ...
    return (
        <div className="h-screen card relative">
            <div className="h-full bgblur bgOpacity card">
                    ...
                    {selectedFolder ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {sortedPhotos.map((photo) => (
                                <Card key={photo.id} className="card bgblur bgOpacity cursor-pointer" onClick={() => openPreview(photo)}>
                                    <div className="aspect-square relative overflow-hidden rounded-lg mb-2 hover:ring-2 ring-primary">
                                    <img
                                        src={photo.url}
                                        alt={photo.name}
            ...
            {previewPhoto && (
                ...
                <div className="relative w-full h-full max-w-[90vw] max-h-[90vh] flex items-center justify-center">
                    ...
                    <Button className="rounded-full p-2 bgOpacity" onClick={() => handleZoom('in')}>
                        <FiZoomIn className="lucidBarIcon" />
                    </Button>
                    <Button className="rounded-full p-2 bgOpacity" onClick={() => handleZoom('out')}>
                        <FiZoomOut className="lucidBarIcon" />
                    </Button>
                    <Button className="rounded-full p-2 bgOpacity" onClick={handleRotate}>
                        <FiRotateCw className="lucidBarIcon" />
                    </Button>
                    <Button className="rounded-full p-2 bgOpacity" onClick={handleDownload}>
                        <FiDownload className="lucidBarIcon" />
                    </Button>
                    <Button className="rounded-full p-2 bgOpacity" onClick={handleDelete}>
                        <FiTrash2 className="lucidBarIcon" />
                    </Button>
                    <Button className="rounded-full p-2 bgOpacity" onClick={closePreview}>
                        <FiX className="lucidBarIcon" />
                    </Button>
                    </div>
                    <Button
                    className="absolute top-1/2 left-4 rounded-full p-2 bgOpacity"
                    onClick={() => navigatePreview('prev')}
                    >
                    <FiChevronLeft className="lucidBarIcon" />
                    </Button>
                    <Button
                    className="absolute top-1/2 right-4 rounded-full p-2 bgOpacity"
