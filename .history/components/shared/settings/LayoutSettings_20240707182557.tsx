...
const LayoutSettings = () => {
    const { taskbarType, setTaskbarType, topbarType, setTopbarType } = useAppStore();
            ...
            <div className="flex flex-row gap-3">
                {themes.map((t) => (
                    <Button
                        key={t}
                        onClick={() => setTaskbarType(getImageIndex(t))}
                        className={`card bgOpacity cursor-pointer h-20 overflow-hidden hover:ring-2 ${top === getImageIndex(t) ? 'ring-2 ring-primary' : ''}`}
                    >
                        <Label className='w-20'>{t}</Label>
                    </Button>
                ))}
