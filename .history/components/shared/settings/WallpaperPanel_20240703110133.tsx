...
const WallpaperPanel = () => {
    ...
    return (
        <div className="grid grid-rows-12 w-full max-h-full">
            <h2 className="text-xl font-bold mb-4 row-span-1">Choose Wallpaper</h2>
            <div className="grid grid-cols-4 gap-3 overflow-y-scroll p-1 max-h-full row-span-11">
                {wallpapers.map((wallpaper, index) => (
                    <div 
                        key={index} 
                        className={` card bgOpacity cursor-pointer h-48 overflow-hidden ${selectedWallpaper === wallpaper ? 'ring-2 ring-primary' : ''}`}
