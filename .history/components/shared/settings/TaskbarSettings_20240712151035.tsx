        ...
        <div className="flex flex-col w-full max-h-full p-1">
            ...
            <div className="flex flex-row gap-3 flex-wrap">
                {directions.map((t) => (
                    <Button
                        key={t}
                        onClick={() => setTaskbarDir(getImageIndex(t, positions))}
                        className={`card bgOpacity cursor-pointer h-20 overflow-hidden hover:ring-2 ${taskbarDir === getImageIndex(t, positions) ? 'ring-2 ring-primary' : ''}`}
                    >
                        <Label className='w-20'>{t}</Label>
                    </Button>
                ))}
