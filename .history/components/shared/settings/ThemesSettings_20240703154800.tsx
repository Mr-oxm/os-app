...
const ThemesSettings = () => {
    ...
    const ThemeButton=({themeName}:{themeName:string})=>{return(
        <Button className={`overflow-hidden !p-0 card bgOpacity w-1/5 h-48 cursor-pointer flex flex-col justify-start ${themeName} text-foreground ${theme===themeName? 'ring-2 ring-primary': ""}`} onClick={() => setTheme(themeName)}>
