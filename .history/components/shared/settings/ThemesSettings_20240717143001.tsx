...
const ThemesSettings = () => {
    ...
    const ThemeButton=({themeName}:{themeName:string})=>{return(
        <Button className={`overflow-hidden text-xs !p-0 card bgOpacity w-full md:w-2/5 h-fit cursor-pointer flex flex-col justify-start ${themeName} text-foreground hover:ring-2 ${sysTheme===themeName? 'ring-2 ring-primary': ''}`} onClick={() => setTheme(themeName)}>
