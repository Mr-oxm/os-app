...
const WallpaperPanel = () => {
    ...
    return (
        <div className="flex flex-col w-fi">
            <h2 className="text-xl font-bold mb-4">Choose Wallpaper</h2>
            <div className="flex flex-row flex-wrap gap-4">
                {wallpapers.map((wallpaper, index) => (
                    <div 
                        key={index} 
                        className={`relative card bgOpacity cursor-pointer h-48 w-48 overflow-hidden ${selectedWallpaper === wallpaper ? 'ring-2 ring-primary' : ''}`}
