...
export default function MainTaskbar() { 
    ...
    const getIconClass = (index:any) => {
        ...
        if (index === hoverIndex) return `scale-[2] ${taskbarDir === 0 ? "mb-6 px-4" : "my-4"}`;
        if (index === hoverIndex - 1 || index === hoverIndex + 1) return `scale-[1.75] ${taskbarDir === 0 ? "mb-4 px-4" : "my-4"}`;
        if (index === hoverIndex - 2 || index === hoverIndex + 2) return `scale-[1.5] ${taskbarDir === 0 ? "mb-2 px-2" : "my-2"}`;
        return "";
    };
