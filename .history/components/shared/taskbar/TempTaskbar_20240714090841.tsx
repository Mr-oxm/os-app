...
const OXMTaskbar = () => {
    ...
    return (
        <div className="absolute w-fit bg-white/60 h-22 bottom-2 transition-all bgblur rounded-full duration-200 group/show hover:w-fit hover:h-12 px-4 hover:py-1 ease-in-out">
            <div className="opacity-0 group-hover/show:opacity-100 group-hover/show:w-fit flex transition-all flex-row gap-2 h-full ease-linear ">
                {TaskbarIcons.map((icon, index) => (
                    ...
                    >
                        ...
                        <span className={`${
                            openedPrograms.some(p => p.id === icon.id) ? '' : 'hidden'}  absolute -bottom-1 group-hover/button:-bottom-3 w-1 h-1 bg-foreground rounded-full`}></span>
                    </button>
                ))}
            </div>
        </div>
