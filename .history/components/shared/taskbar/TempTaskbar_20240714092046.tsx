            ...
            <div className="opacity-0 group-hover/show:opacity-100 group-hover/show:w-fit flex transition-all flex-row gap-2 h-full ease-linear ">
                {TaskbarIcons.map((icon, index) => (
                    <div
                        ...
                        className={`transition-all group/button ease-linear relative duration-100 mb-0 hover:mb-3 active:scale-150 
                        }`}
                    >
                        <OXMDockIcon imgSrc={icon.imgSrc[iconsType]} name={icon.name} />
                        <span className={`${
                            openedPrograms.some(p => p.id === icon.id) ? '' : 'hidden'}  absolute -bottom-0 group-hover/button:-bottom-2 transition-all w-1 h-1 bg-foreground rounded-full text-center`}></span>
                    </div>
                ))}
            </div>
        </div>
