            ...
            <div className={`opacity-0 group-hover/show:opacity-100  flex transition-all  gap-2  ease-linear ${taskbarDir===0?"flex-row h-full group-hover/show:w-fit":"flex-col"}`}>
                {TaskbarIcons.map((icon, index) => (
                    ...
                    >
                        ...
                        <span className={`${
                            openedPrograms.some(p => p.id === icon.id) ? '' : 'hidden'}  absolute icon-buffer  transition-all w-1 h-1 bg-foreground rounded-full ${taskbarDir===0 && "-bottom-0 group-hover/button:-bottom-2"}
                        }`}></span>
                    </div>
                ))}
            </div>
