...
export default function MainTaskbar() { 
    ...
    const getIconClass = (index:any) => {
        ...
        if (index === hoverIndex) return "scale-[2] "+ (taskbarDir===0? "mb-6":"");
        if (index === hoverIndex - 1 || index === hoverIndex + 1) return "scale-[1.75]" + (taskbarDir===0? "mb-4":"");
        if (index === hoverIndex - 2 || index === hoverIndex + 2) return "scale-[1.5]"+ (taskbarDir===0? "mb-3":"");
        return "";
    };
    return (
        <div className={`ease-in-out bgOpacity bgblur !p-1 m-2 my-1 flex ${taskbarDir===0? "flex-row h-12 hover:gap-8":"flex-col w-12 hover:gap-3"}  gap-2  transition-all card`}>
