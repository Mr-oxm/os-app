...
export default function MainTaskbar() { 
    ...
    const getIconClass = (index) => {
        ...
        if (diff === 0) return `${baseClass} !scale-[2] ${taskbarDir === 0 ? "mb-6 mx-4" : "my-4"}`;
        if (diff === 1) return `${baseClass} !scale-[1.5] ${taskbarDir === 0 ? "mb-4 mx-4" : "my-4"}`;
        if (diff === 2) return `${baseClass} !scale-[1.25] ${taskbarDir === 0 ? "mb-2 mx-2" : "my-2"}`;
        return baseClass;
    };
