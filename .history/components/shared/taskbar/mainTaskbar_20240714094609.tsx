...
export default function MainTaskbar() { 
    ...
    return (
        <div className={`ease-in-out bgOpacity bgblur !p-1 m-2 my-1 flex ${taskbarDir === 0 ? "flex-row h-12" : "flex-col w-12"}  gap-2  transition-all card z-50`}>
            {TaskbarIcons.map((icon, index) => (
                <div
                    key={index}
                    onClick={() => handleProgram(icon.id)}
                    className={`transition-all ease-linear relative duration-100 group/show ${getIconClass(index)}`}
                ...
                >
                    ...
                    } absolute group-hover/show:hidden -bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-foreground rounded-full`}></span>
                </div>
            ))}
        </div>
    );
