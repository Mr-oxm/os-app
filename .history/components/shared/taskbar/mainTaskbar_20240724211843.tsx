...
export default function MainTaskbar() { 
    ...
    const getIconClass = (index:any) => {
        const baseClass = "transition-all duration-300 ease-in-out flex items-center justify-center";
        if (hoverIndex === null) return baseClass;
        const diff = Math.abs(index - hoverIndex);
        if (diff === 0) return `${baseClass} !scale-[2] ${taskbarDir === 0 ? "mb-6 px-4" : "my-4"}`;
        ...
        <div className={`ease-in-out bgOpacity bgblur !p-1 m-2 my-1 flex ${taskbarDir === 0 ? "flex-row h-12" : "flex-col w-12"}  gap-2  transition-all card z-50 group/show `}>
            {TaskbarIcons.map((icon, index) => (
                ...
                >
                    ...
                    } absolute group-hover/show:opacity-0 opacity-100 transition-all duration-300 -bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-foreground rounded-full`}></span>
                </div>
            ))}
        </div>
    );
