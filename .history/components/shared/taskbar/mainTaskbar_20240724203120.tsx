...
export default function MainTaskbar() { 
    ...
    const getIconClass = (index:any) => {
        ...
        if (index === hoverIndex - 1 || index === hoverIndex + 1) return `scale-[1.75] ${taskbarDir === 0 ? " mx-4" : "my-4"}`;
        if (index === hoverIndex - 2 || index === hoverIndex + 2) return `scale-[1.5] ${taskbarDir === 0 ? " mx-2" : "my-2"}`;
        return "";
    };
