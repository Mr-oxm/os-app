...
export default function MainTaskbar() { 
    ...
    const getIconClass = (index) => {
        ...
        if (index === hoverIndex) return `${baseClass} scale-200 ${taskbarDir === 0 ? "mb-6 px-4" : "my-6"}`;
        if (index === hoverIndex - 1 || index === hoverIndex + 1) return `${baseClass} scale-175 ${taskbarDir === 0 ? "mb-4 px-3" : "my-4"}`;
        if (index === hoverIndex - 2 || index === hoverIndex + 2) return `${baseClass} scale-150 ${taskbarDir === 0 ? "mb-2 px-2" : "my-2"}`;
        return baseClass;
    };
