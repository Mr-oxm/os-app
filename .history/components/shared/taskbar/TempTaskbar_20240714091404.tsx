...
const OXMTaskbar = () => {
    ...
    return (
        <div className="absolute w-fit bg-white/60 h- bottom-2 transition-all bgblur rounded-full duration-200 group/show hover:w-fit hover:h-14 px-4 hover:py-1 ease-in-out">
            <div className="opacity-0 group-hover/show:opacity-100 group-hover/show:w-fit flex transition-all flex-row gap-2 h-full ease-linear ">
                {TaskbarIcons.map((icon, index) => (
                    <button
                        ...
                        className={`transition-all group/button ease-linear relative duration-100 mb-0 hover:mb-3 active:scale-125 
                        }`}
                    >
                        <OXMDockIcon imgSrc={icon.imgSrc[iconsType]} name={icon.name} />
                        <span className={`${
                            openedPrograms.some(p => p.id === icon.id) ? '' : 'hidden'}  absolute -bottom-0 group-hover/button:-bottom-2 w-3 right-1/2 left-1/2 h-1 bg-foreground rounded-full`}></span>
                    </button>
                ))}
            </div>
        </div>
