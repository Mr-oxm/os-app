            ...
            <div className={`opacity-0 group-hover/show:opacity-100  flex transition-all  gap-2  ease-linear ${taskbarDir===0?"flex-row h-full group-hover/show:w-fit":"flex-col"}`}>
                {TaskbarIcons.map((icon, index) => (
                    ...
                    >
                        ...
                        <span className={`${
                            openedPrograms.some(p => p.id === icon.id) ? '' : 'hidden'}  absolute  transition-all w-1 h-1 bg-foreground rounded-full ${taskbarDir===0?"hover:mb-3":(taskbarDir===1?"hover:mr-3":"hover:ml-3")}
                        }`}></span>
                    </div>
                ))}
            </div>
