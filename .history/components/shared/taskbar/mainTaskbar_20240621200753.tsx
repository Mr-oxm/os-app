...
export default function MainTaskbar() {
    ...
    return (
        <div className="ease-in-out bg-black/15 backdrop-blur-sm p-2 rounded-xl flex flex-row gap-2 hover:gap-10 transition-all border-2 border-border/20">
            {icons.map((icon, index) => (
                <div className={`transition-all ease-in-out duration-300 ${getIconClass(index)}`}onMouseEnter={() => handleMouseEnter(index)}
