            ...
            <div className={`opacity-0 group-hover/show:opacity-100  flex transition-all  gap-2  ease-linear ${taskbarDir===0?"flex-row h-full group-hover/show:w-fit":"flex-col"}`}>
                {TaskbarIcons.map((icon, index) => (
                    <div
                        ...
                        className={`transition-all group/button ease-linear relative duration-100 mb-0  active:scale-150 icon-buffer del text-center ${taskbarDir===0?"hover:mb-3":(taskbarDir===1?"hover:mr-3":"hover:ml-3")}
                        }`}
                    >
