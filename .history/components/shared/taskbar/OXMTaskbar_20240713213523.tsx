...
const OXMTaskbar = () => {
    ...
    return (
        <div className="absolute w-44 bg-white/60 h-4 bottom-2 transition-all bgblur rounded-full duration-200 group/show hover:w-fit hover:h-12 px-4 hover:py-2 ease-in-out">
            <div className="group-hover/show:flex transition-all flex-row gap-2 hidden h-full ">
                {TaskbarIcons.map((icon, index) => (
                    <Link href={icon.route} className={` transition-all ease-linear relative duration-100 mb-0 hover:mb-7 `}
                    >
