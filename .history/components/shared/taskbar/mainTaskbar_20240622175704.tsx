...
export default function MainTaskbar() {
    ...
    const getIconClass = (index:any) => {
        ...
        if (index === hoverIndex) return "scale-[2] bottom-8";
        if (index === hoverIndex - 1 || index === hoverIndex + 1) return "scale-[1.75]";
        if (index === hoverIndex - 2 || index === hoverIndex + 2) return "scale-[1.5]";
        return "";
    };
    return (
        <div className="ease-in-out bg-black/15 backdrop-blur-sm p-1 m-2 rounded-xl flex flex-row gap-2 hover:gap-8 transition-all border-2 border-border/20">
            {icons.map((icon, index) => (
                <div className={` transition-all ease-linea relativer duration-100 ${getIconClass(index)}`}onMouseEnter={() => handleMouseEnter(index)}
