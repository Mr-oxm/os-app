...
export default function MainTaskbar() { 
    ...
    const getIconClass = (index:any) => {
        ...
        if (index === hoverIndex) return `scale-[2] ${taskbarDir === 0 ? " px-2" : "py-4"}`;
        if (index === hoverIndex - 1 || index === hoverIndex + 1) return `scale-[1.75] ${taskbarDir === 0 ? " px-4" : "py-4"}`;
        if (index === hoverIndex - 2 || index === hoverIndex + 2) return `scale-[1.5] ${taskbarDir === 0 ? " px-2" : "py-2"}`;
        return "";
    };
        ...
        <div className={`ease-in-out bgOpacity bgblur !p-1 m-2 my-1 flex ${taskbarDir === 0 ? "flex-row h-12" : "flex-col w-12"}  gap-2  transition-all card z-50 group/show`}>
            {TaskbarIcons.map((icon, index) => (
                ...
                >
                    ...
                    } absolute group-hover/show:opacity-0 opacity-100 transition-all -bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-foreground rounded-fullv hover:mb-6`}></span>
                </div>
            ))}
        </div>
    );
