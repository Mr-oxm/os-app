...
export default function MainTaskbar() {
    ...
    const getIconClass = (index:any) => {
        ...
        if (index === hoverIndex - 1 || index === hoverIndex + 1) return "scale-[1.75] mb-6";
        if (index === hoverIndex - 2 || index === hoverIndex + 2) return "scale-[1.5] mb-6";
        return "";
    };
    return (
        <div className="ease-in-out bg-black/15 backdrop-blur-sm p-1 m-2 rounded-xl flex flex-row gap-2 hover:gap-8 transition-all border-2 border-border/20 hover:h-14">
