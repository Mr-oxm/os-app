...
const OXMTaskbar = () => {
    ...
    return (
        <div className={`absolute w-fit bg-white/60 h-4 bottom-2 transition-all bgblur rounded-full duration-200 group/show hover:w-fit hover:h-14 px-4 hover:py-1 ease-in-out z-50 ${taskbarDir===0?"flex-row h-full group-hover/show:w-fit":""}`}>
            <div className={`opacity-0 group-hover/show:opacity-100  flex transition-all  gap-2  ease-linear ${taskbarDir===0?"flex-row h-full group-hover/show:w-fit":""}`}>
                {TaskbarIcons.map((icon, index) => (
                    <div
                        key={index}
                        onClick={() => handleProgram(icon.id)}
                        className={`transition-all group/button ease-linear relative duration-100 mb-0 hover:mb-3 active:scale-150 text-center 
                        }`}
                    >
