...
const OXMTaskbar = () => {
    ...
    return (
        <div className="absolute w-fit bg-white/60 h-4 bottom-2 transition-all bgblur rounded-full duration-200 group/show hover:w-fit hover:h-12 px-4 hover:pxy-11 ease-in-out">
                ...
                {TaskbarIcons.map((icon, index) => (
                    <button
                        ...
                        className={`transition-all ease-linear relative duration-100 mb-0 hover:mb-3 active:scale-125 ${
                            openedPrograms.some(p => p.id === icon.id) ? 'border-2 border-blue-500' : ''
                        }`}
                    >
