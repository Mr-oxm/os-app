...
const OXMTaskbar = () => {
    ...
    return (
        <div className={`absolute  bg-white/60 transition-all bgblur rounded-full duration-200 group/show ease-in-out z-50 ${taskbarDir===0?" hover:w-fit hover:h-14 px-4 hover:py-1 h-4 bottom-2 w-fit":"w-4 h-fit hover:w-14 py-4 "} ${taskbarDir===1? "right-2":"left-2"}  `}>
            <div className={`opacity-0 group-hover/show:opacity-100  flex transition-all  gap-2  ease-linear ${taskbarDir===0?"flex-row h-full group-hover/show:w-fit":"flex-col"}`}>
                {TaskbarIcons.map((icon, index) => (
                    <div
                        ...
                        className={`transition-all group/button ease-linear relative duration-100 mb-0  active:scale-150 text-center ${taskbarDir===0?"hover:mb-3":(taskbarDir===1?"hover:mr-3":"hover:ml-3")}
                        }`}
                    >
                        <OXMDockIcon imgSrc={icon.imgSrc[iconsType]} name={icon.name} />
                        <span className={`${
                            openedPrograms.some(p => p.id === icon.id) ? '' : 'hidden'}  absolute -bottom-0 group-hover/button:-bottom-2 transition-all w-1 h-1 bg-foreground rounded-full ${taskbarDir===0?"hover:mb-3":(taskbarDir===1?"hover:mr-3":"hover:ml-3")}
                        }`}></span>
                    </div>
                ))}
            </div>
