    ...
    return (
        <div className="w-screen bg-background/80 backdrop-blur-md border-t border-border max-h-14 flex flex-row items-center justify-between px-2"> 
                                ...
                                <div className="col-span-2 w-full">
                                    ...
                                    <div className="flex flex-row gap-2 h-24 w-full">
                                        {TaskbarIcons.map((icon, index) => (
                                            ...
                                            > 
                                                <Button variant="ghost" className={`p-6 border-transparent w-full h-full  group border hover:border-accent/25 hover:bg-accent/40 flex flex-col`} >
                                                    <Image 
                ...
                <Popover>
                    <PopoverTrigger>
                        {/* System Tray */}
                        <Button variant="ghost" className="p-1.5 border-transparent group border hover:border-accent/25 hover:bg-accent/40 flex flex-row gap-2">
...
import { Button, Popover, PopoverContent, PopoverTrigger, Separator, Slider, Label } from "@/components/ui";
import { Wifi, Bluetooth, Plane, BatteryLow, Accessibility, Monitor, Moon, Battery, Pencil, Settings } from "lucide-react";
    ...
    <PopoverContent className="w-80 p-0">
      <div className="grid grid-cols-3 gap-2 p-4">
        {quickSettings.map(({ icon: Icon, label }) => (
          <Button key={label} variant="ghost" className="flex flex-col items-center justify-center h-24">
