...
export default function MainTaskbar() {
    ...
    const getIconClass = (index:any) => {
        ...
        if (index === hoverIndex) return "scale-150";
        if (index === hoverIndex - 1 || index === hoverIndex + 1) return "scale-125";
        if (index === hoverIndex - 2 || index === hoverIndex + 2) return "scale-110";
        return "";
    };
    return (
        <div className="ease-in-out bg-black/15 backdrop-blur-sm p-2 rounded-xl flex flex-row gap-2 hover:gap-8 transition-all border-2 border-border/20">
            {icons.map((icon, index) => (
                <div>
                    <DockDesktopIcon className={`${getIconClass(index)}`} key={index} imgSrc={icon.imgSrc} name={icon.name} onMouseEnter={() => handleMouseEnter(index)}
