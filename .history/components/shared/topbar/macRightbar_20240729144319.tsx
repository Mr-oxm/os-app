...
const MacRightbar = () => {
    ...
    return (
        <div className="flex flex-row-reverse h-5/6">
            {menuItems.map((item, index) => (
                <DropdownMenu key={index}>
                    <DropdownMenuTrigger className={`bg-transparent py-0 px-2 h-full !text-foreground !text-xs hover:bg-background/40 rounded-2xl ${item.className || ''}`}>
