...
const LinuxTopbar = () => {
    ...
    const { openProgram, openedPrograms, minimizeProgram, maximizeProgram, active, setActive } = useOSMemoryStore();
    ...
    useEffect(() => {
        const active = openedPrograms.find(program => !program.minimized);
        ...
        <div className={`bg-background/80 backdrop-blur-md flex flex-row justify-between h-6 px-4 w-screen py-1`}> 
                ...
                <Dialog>
                    <DialogTrigger className="capitalize p-1.5 border-transparent group border hover:border-accent/25 hover:bg-accent/40 flex flex-row h-full gap-2 items-center rounded-md">
                        {activeProgram ? activeProgram.name : "Applications"}
                    </DialogTrigger>
