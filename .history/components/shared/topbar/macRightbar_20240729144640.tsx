        ...
        <div className="flex flex-row-reverse h-full p-1 ">
            {menuItems.map((item, index) => (
                <DropdownMenu key={index}>
                    <DropdownMenuTrigger className={`bg-transparent !py-0 !px-2 h-full !text-foreground !text-xs hover:bg-background/40 rounded-xl transition-all ease-in-out !border-transparent hover:!border-border card ${item.className || ''}`}>
