...
const OXMTopbar = () => {
    ...
    return (
        <div className={`bg-background/80 backdrop-blur-md flex flex-row justify-between h-6 px-4 py-1 z-50 rounded-full w-[calc(100%-8px)] my-1`}> 
                ...
                <Popover>
                    <PopoverTrigger className="capitalize p-1.5 border-transparent group border hover:border-background/25 hover:bg-background/40 flex flex-row h-full gap-2 items-center rounded-md">
                        <IoIosApps /> 
                    </PopoverTrigger>
                    <PopoverContent className="card bgblur bg-background/80 ml-5 mt-1 grid grid-cols-2">
                        ...
                        <div className="flex flex-col items-center justify-start gap-2">
                            {TaskbarIcons.map((icon, index) => ( 
                                    <Button key={index} onClick={() => handleProgram(icon.id)} variant="ghost" className={`!p-1.5 flex flex-row gap-3 card !border-transparent w-full h-12 group border hover:!border-background/25 hover:bg-primary  justify-start active:scale-110 transition-transform ${
                ...
                <Popover>
                        <PopoverTrigger className="p-1.5 border-transparent group border hover:border-background/25 hover:bg-background/40 flex flex-row h-full gap-2  items-center rounded-md">
                            {formattedDate}
                        </PopoverTrigger>
                ...
                <div className={`flex justify-end flex-row w-1/5 h-full`}> 
                    <Popover>
                        <PopoverTrigger>
                            {/* System Tray */}
                            <div className={`p-1.5 border-transparent group border hover:border-background/25 hover:bg-background/40 flex flex-row h-full gap-2  items-center rounded-md`}>
