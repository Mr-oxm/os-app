...
const OXMenu=({handleProgram}:{handleProgram:Function})=>{
    ...
    const { openProgram, openedPrograms, minimizeProgram, maximizeProgram, active } = useOSMemoryStore();
    return(
        <PopoverContent className="card bgblur bg-background/80 ml-5 mt-1 grid grid-cols-6 gap-2 w-96 ">
            {/* left section  */}
            <div className="flex flex-col justify-between gap-2 col-span-5">
                <ScrollArea className="flex flex-col items-center justify-start gap-2">
                    {TaskbarIcons.map((icon, index) => ( 
                            <Button key={index} onClick={() => handleProgram(icon.id)} variant="ghost" className={`!p-1.5 flex flex-row gap-3 card  w-full h-12 group !border-transparent border hover:!border-border/50 hover:bg-primary  justify-start active:scale-110 transition-transform ${
                ...
                <Input className="text-xs h-9 card !shadow-none" placeholder={`Search`}/>
            </div>
            {/* right section  */}
            <div className="flex flex-col justify-between text-xs py-1 items-center">
