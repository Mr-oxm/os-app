    ...
    return (
        <div className={`bg-background/80 backdrop-blur-md flex flex-row justify-between h-6 px-4 py-1 z-50 rounded-full w-[calc(100%-8px)] my-1`}> 
                ...
                <Popover>
                    ...
                    <PopoverContent className="card bgblur bg-background/80 ml-5 grid grid-cols-2">
                        
                        <div className="flex flex-col items-center justify-start gap-2">
                            {TaskbarIcons.map((icon, index) => ( 
                                    <Button key={index} onClick={() => handleProgram(icon.id)} variant="ghost" className={`!p-1.5 flex flex-row gap-3 card !border-transparent w-full h-12 group border hover:!border-accent/25 hover:bg-primary  justify-start active:scale-110 transition-transform ${
                                        openedPrograms.some(p => p.id === icon.id) ? 'bg-primary' : 'hover:bg-primary'
                                    }`}>
                                        <Image 
                                        ...
                                        />
                                        <Label className="text-xs hover:text-primary-foreground">{icon.name}</Label>
                                    </Button> 
                            ))} 
                        </div>
                    </PopoverContent>
