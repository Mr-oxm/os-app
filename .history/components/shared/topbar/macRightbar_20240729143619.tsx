        ...
        <div className="flex flex-row-reverse h-full">
            {menuItems.map((item, index) => (
                <DropdownMenu key={index}>
                    <DropdownMenuTrigger className={`bg-transparent py-0 px-2 h-full !text-foreground !text-xs hover:bg-background/40 ${item.className || ''}`}>
                        {item.icon || item.label}
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className="p-2 w-auto md:w-80 text-foreground card bgOpacity bgblur flex">
                        <DropdownMenuItem>{item.content}</DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
            ))}
        </div>
