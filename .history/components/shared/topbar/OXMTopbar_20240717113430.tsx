...
function SystemTrayMenu() {
    return (
    <PopoverContent className="w-80 card bgOpacity backdrop-blur-md mt-1 overflow-hidden flex flex-col gap-3">
        <div className="grid grid-cols-3 gap-2 card bgOpacity">
            {quickSettings.map(({ icon: Icon, label }) => (
            <div className="flex flex-col items-center gap-2">
                <Button key={label} variant="ghost" className="card bgOpacity cursor-pointer h-20 overflow-hidden hover:ring-2 w-full flex flex-col gap-2 group active:ring-2 active:ring-primary">
                ...
                <ScrollArea className="max-h-72">
                    <div className="!flex !flex-col gap-1 items-center justify-start min-h-64">
                        {filteredIcons.map((icon, index) => ( 
                            <Button key={index} onClick={() => handleProgram(icon.id)} variant="ghost" className={`!p-1.5 flex flex-row gap-3 card  w-full h-10 group bgOpacity hover:!bg-foreground/10 ease-in-out justify-start active:scale-110 transition-all drop-shadow-none ${
