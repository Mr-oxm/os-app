...
const LinuxTopbar = () => {
    ...
    return (
        <div className={` bg-background/80 backdrop-blur-md  flex flex-row justify-between h-6 px-4 w-screen py-1 `}> 
                ...
                <Dialog>
                    <DialogTrigger>{programTitle?programTitle:"Desktop"}</DialogTrigger>
                    <DialogContent className="flex flex-col items-center w-fit bgblur bgOpacity">
                        <Label>Applications</Label>
                        <div className="flex flex-row flex-wrap items-center just gap-4">
                            {TaskbarIcons.map((icon, index) => (
                                <Link href={icon.route} 
                                    
                                > 
                                    <Button variant="ghost" className={`p-1.5 border-transparent w-16 h-16 group border hover:border-accent/25 hover:bg-accent/40 'bg-accent/40 hover:hover:bg-accent/40
                                    }`} >
                                        <Image 
