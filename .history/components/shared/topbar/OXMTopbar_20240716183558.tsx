            ...
            <div className="flex flex-col justify-between gap-2 col-span-5 bgOpacity card">
                <ScrollArea className="!flex !flex-col items-center justify-start">
                    {TaskbarIcons.map((icon, index) => ( 
                            <Button key={index} onClick={() => handleProgram(icon.id)} variant="ghost" className={`!p-1.5 mb-1 flex flex-row gap-3 card  w-full h-8 group bgOpacity hover:!bg-foreground/20 justify-start active:scale-110 transition-transform ${
