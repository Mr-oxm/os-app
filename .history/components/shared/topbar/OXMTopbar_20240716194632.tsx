...
const sliders = [
    { id: "brightness", Icon: TiAdjustBrightness },
...
function SystemTrayMenu() {
    ...
    <PopoverContent className="w-80 card bgOpacity backdrop-blur-md mt-1 overflow-hidden flex flex-col gap-3">
        ...
        <div className="card bgOpacity grid grid-cols-2 gap-2 h-48">
            {sliders.map(({ id, Icon }) => (
            <div key={id} className="flex flex-col gap-2 items-center h-full col-span-1">
                <Slider id={id} className="my-2 -rotate-90" />
                <Icon className="h-4 w-4" />
            </div>
            ))}
        </div> 
