...
const OXMenu=({handleProgram}:{handleProgram:Function})=>{
    ...
    const { openProgram, openedPrograms, minimizeProgram, maximizeProgram, active } = useOSMemoryStore();
    return(
        <PopoverContent className="card bgblur bgOpacity ml-5 mt-1 grid grid-cols-7 gap-2 w-96 ">
            {/* left section  */}
            <div className="flex flex-col justify-between gap-2 col-span-6 bgOpacity card">
                <ScrollArea className="!flex !flex-col items-center justify-start">
                    {TaskbarIcons.map((icon, index) => ( 
                            <Button key={index} onClick={() => handleProgram(icon.id)} variant="ghost" className={`!p-1.5 mb-1 flex flex-row gap-3 card  w-full h-10 group bgOpacity hover:!bg-foreground/20 justify-start active:scale-110 transition-transform ${
            ...
            {/* right section  */}
            <div className="flex flex-col justify-between text-xs !py-1 items-center card bgOpacity">
