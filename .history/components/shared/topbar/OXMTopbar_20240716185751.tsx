    ...
    return(
        <PopoverContent className="card bgblur bgOpacity ml-4 mt-1 grid grid-cols-6 gap-2 w-96 ">
            ...
            <div className="flex flex-col justify-between gap-2 col-span-5 bgOpacity card">
                <Label className="ml-2 font-bold text-sm">All apps</Label>
                <ScrollArea className="!flex !flex-col items-center justify-start max-h-72">
                    {TaskbarIcons.map((icon, index) => ( 
                            <Button key={index} onClick={() => handleProgram(icon.id)} variant="ghost" className={`!p-1.5 mb-1 flex flex-row gap-3 card  w-full h-10 group bgOpacity hover:!bg-foreground/10 ease-in-out justify-start active:scale-110 transition-all ${
                                openedPrograms.some(p => p.id === icon.id) ? 'bg-primary !text-primary-foreground' : ''
                            }`}>
                                <Image 
                ...
                <Input className="text-xs h-9 card !drop-shadow-none !shadow-none bg-background" placeholder={`Search`}/>
            </div>
            {/* right section  */}
            <div className="flex flex-col justify-between text-xs !px-1 items-center card bgOpacity active:outline-">
