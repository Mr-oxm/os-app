...
const MainLockScreen = ({ wallpaper, setIsLocked }: { wallpaper: string; setIsLocked: Function }) => {
    ...
    useEffect(() => {
        if (isUnlocking) {
            const timer = setTimeout(() => {
                setIsLocked(false);
            }, 1000); // Delay to allow for exit animation
    ...
    return (
        <main 
            className={`cursor-macos flex h-screen w-screen flex-col items-center justify-between max-h-screen max-w-screen overflow-hidden bg-cover bg-center animate-fadeIn ${isUnlocking ? 'animate-fadeOut' : ''}`}
        ...
        >
            <div className='bgOpacity bgblur w-full h-full flex flex-col items-center gap-4 justify-center'>
                ...
                <Button 
                    ...
                    className='bgBlur bgOpacity text-foreground w-24 card hover:!bg-primary hover:!text-primary-foreground transition-all duration-300 hover:scale-105 active:scale-95 animate-fadeIn'
                >
                    Enter
                </Button>
            </div>
